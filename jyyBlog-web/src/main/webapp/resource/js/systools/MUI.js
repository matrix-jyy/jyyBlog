...
return {
	...
	*/
	//改变地区选择
	 changArea:function(node,cityUrl) {
		 var id = $(node).val();
		if (id == -1) {
			return;
		}
		$.AjaxProxy({p : {id : $(node).val(),},a:false}).invoke(cityUrl,function(loj) {
			var html="";
			if ($(node).next().next().is('select')) {
				html = "<option value=-1>--城市--</option>";
				$(node).next().next().hide();
				$(node).next().next().removeAttr("datatype");
			}
			else{
				if(loj.getRowCount()>0){
				html = "<option value=-1>--地区--</option>";
				$(node).next().attr("datatype","s");
				$(node).next().show();
				}
			}
	...
	//初始化地区
	 initdq:function(id,provUrl,childUrl) {
        console.log(id);
		 $.AjaxProxy({a:false}).invoke(provUrl,function(loj){
				var html = "<select class='select select-box areaBox margin2 col-sm-3' name='province' onchange='MUI.changArea(this,\""+childUrl+"\")' dataType='s' nullmsg='省不能为空'>" + "<option value='-1'>--省份--</option>";
				for (var i = 0; i < loj.getRowCount(); i++) {
					html = html + '<option value=' + loj.getString(i, "id") + ' >' + loj.getString(i, "name") + '</option>';
				}
				html += "</select>" 
						+"<select class='select select-box areaBox margin2 col-sm-3' name='city' id='city'  onchange='MUI.changArea(this,\""+childUrl+"\")' dataType='s' nullmsg='市不能为空'>" + " <option value='-1' >--城市--</option></select>" 
						+"<select style='display:none' class='select select-box areaBox margin2 col-sm-3' name='quyu' id='quyu'>" + "<option value='-1' >--地区--</option>" + "</select>"
				$("#"+id).empty().html(html);
			});
	},
	 ...
	 * @param fileTypes:文件支持的上传类型默认为gif,jpg,jpeg,png,bmp。可以通过-data-fileTypes改变
	 */
		...
		$(selected).each(function(i){
			...
			file.change(function(){
			    ...
			    if(fileTypes.indexOf(photoExt)<0){
			       layer.msg("允许的文件类型是1："+fileTypes,{icon:2,time:2000});
			        return false;
			    }
				...
				if($input.next().next().is("img")){
					...
					$input.next("a").after("<img onclick='MUI.removeUpload(this,\""+hideInputId+"\")'  class='loadingbig' src='"+basePath+"/resource/images/loadingbig.gif' />")
				}
				//提交表单
				form.submit();
			});
